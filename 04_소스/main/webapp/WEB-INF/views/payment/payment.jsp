...
#paymenttbl td:nth-child(2), #paymenttbl td:nth-child(3), #paymenttbl td:nth-child(4),
	#paymenttbl td:nth-child(5), #paymenttbl td:nth-child(6) {
...
<body>
				...
				<div class="col-8 main-width">
					<div class="over-wrap">
						...
						<div class="my-3 p-3 bg-body rounded shadow-sm">
							...
							<div class="d-flex pt-3">
								<div class="pb-3 mb-0 border-bottom w-100">
									...
									<span class="aligncenter"> <c:forEach items="${dlist}"
											var="dto" varStatus="status">
											<label class="radio-label"> <input type="radio"
												name="deliveryseq" id="dvseq" value="${dto.deliveryseq}"
										...
										<button type="button" class="btn" data-bs-toggle="modal"
											...
											style="-bs-btn-padding-y: .25rem; - -bs-btn-padding-x: .5rem; - -bs-btn-font-size: .75rem; background-color: whitesmoke;">
											신규 배송지 등록</button>
										<button type="button" class="btn" data-bs-toggle="modal"
											data-bs-target="#addressModal2"
											style="-bs-btn-padding-y: .25rem; - -bs-btn-padding-x: .5rem; - -bs-btn-font-size: .75rem; background-color: whitesmoke;">
											배송지 관리</button>
									</span>
								</div>
							...
							<div class="d-flex pt-3">
								<div class="pb-3 mb-0 border-bottom w-100">
									<strong class="text-gray-dark" style="font-size: 14px;">배송
										요청사항</strong> <span class="aligncenter"> <select name=""
										id="selectbox"
										style="border: 1px solid gray; height: 30px; width: 400px; font-size: 13px;">
								...
								<div class="pb-3 mb-0 w-100">
									<table class="table table-bordered" id="paymenttbl">
										...
										<c:forEach items="${bdtoList}" var="bdto">
											<tr>
												<td><img
													src="/resources/image/bookposter/${bdto.image}" alt=""
													style="object-fit: cover; height: 60px"> <span>${bdto.title}</span>
												</td>
												<td>1</td>
						...
						<div class="my-3 p-3 bg-body rounded shadow-sm">
							...
							<div class="d-flex pt-3">
								<div class="pb-3 mb-0 border-bottom w-100">
									<strong class="text-gray-dark" style="font-size: 14px;">상품
										금액</strong> <span class="aligncenter"><span id="price1">${total}</span>원</span>
								</div>
							</div>

							<div class="d-flex pt-3">
								<div class="pb-3 mb-0 border-bottom w-100">
									<strong class="text-gray-dark" style="font-size: 14px;">쿠폰
										할인</strong>
									...
									<span class="aligncenter" class="rate-value"><span
										id="price2">0</span> <span>원</span></span>
									<button type="button" class="btn alignmorecenter"
							...
							<div class="d-flex pt-3">
								<div class="pb-3 mb-0 w-100">
									<strong class="text-gray-dark" style="font-size: 14px;">최종
										결제 금액</strong>
									<%-- <span class="aligncenter">${total - Math.round(total * (dto.rate * 0.01))} 원</span> --%>
									<span class="aligncenter"><span id="price3">${total}</span>
										원</span>
								</div>
							</div>
						</div>
					...
					<div style="padding: 30px">
						<button class="pay btn btn-dark alignright"
							onclick="requestPay();">결제하기</button>
					</div>


					<form method="POST" id="paymentform1" action="/payment/payment">
						<input type="hidden" name="name" value="${membername}"> <input
							type="hidden" name="tel" value="${phonenumber}"> <input
							type="hidden" name="address" value="${address}"> <input
							type="hidden" name="pay" value="${total}"> <input
							type="hidden" name="checknumber" value="23132"> <input
							type="hidden" name="couponseq" value="2"> <input
							type="hidden" name="memberseq"
							value="<sec:authentication property="principal.memberseq"/>">
						<input type="hidden" name="${_csrf.parameterName}"
							value="${_csrf.token }" class="token"> <input
							type="hidden" name="${_csrf.headerName}"
							value="${_csrf.headerName }" class="csrf-header">
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<table class="table">
						...
						<c:forEach items="${dto}" var="dto">
							<tr>
								<td><input type="radio" name="ab" class="cradio"
									value="${dto.couponseq}"></td>
								<td>${dto.couponseq}</td>
								<td>${dto.name}</td>
								<td>${dto.deadline}</td>
								<td>${dto.rate}</td>
				...
				<div class="modal-footer">
					...
					<button type="button" class="btn btn-primary"
						data-bs-dismiss="modal" id="couponBtn">쿠폰 등록</button>
				</div>
			</div>
		</div>
	</div>
	...
	<form method="post" action="/payment/paymentok.do">
						...
						<table class="tbladdress">
							<tr style="margin-bottom: 30px;">
								...
								<td><input type="text" class="input-middle"
									name="membername" id=""></td>
							</tr>
							<tr>
								<th>배송지명</th>
								<td><input type="text" class="input-middle"
									name="addressaname" id=""></td>
							</tr>
							<tr>
								<th>휴대전화</th>
								<td><input type="text" class="input-small" name="phone1"
									id=""> <span>-</span> <input type="text"
									class="input-small" name="phone2" id=""> <span>-</span>
							...
							<tr>
								...
								<td>
									<div>
										<input type="text" class="input-middle" name=""
											id="member_post" readonly> <input
											class="btn btn-dark" type="button" name="" value="검색"
											onclick="findAddr()">
									</div>
									<div>
										<input type="text" class="input-long" name="address1"
											id="member_addr" readonly> <input type="text"
											class="input-long" name="address2" id=""
											placeholder="상세주소 입력">
									</div>
		...
		<input type="hidden" name="memberseq"
			value="<sec:authentication property="principal.memberseq"/>">
		<input type="hidden" name="${_csrf.parameterName}"
			value="${_csrf.token }" class="token"> <input type="hidden"
			name="${_csrf.headerName}" value="${_csrf.headerName }"
			class="csrf-header">
	</form>
	...
	<script>
        var IMP = window.IMP;
        IMP.init("imp61761255"); // 예: imp00000000
        ...
        function requestPay() {
          ...
          IMP.request_pay({ // param
              ...
              if (rsp.success) {
                  // 결제 성공 시 로직
            	  window.location.href = "결제완료페이지URL";
...
/*          
        ...
        function requestPay(){
        	var uid = '';
        	IMP.init('imp61761255') //가맹점 식별코드
        	
        	IMP.request_pay({
        		pg: 'kakaopay' ,
        		pay_method: 'card' ,
        		merchant_uid: createOrderNum() , // 가맹점 주문
        		name:
        		amount:
        		buyer_email:
        		buyer_name:
